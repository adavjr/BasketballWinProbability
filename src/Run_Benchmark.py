...
NOTE: Currently only done on NBA 
"""

import random
import pandas as pd

from DataLoad import load_nba_data
...
from Benchmark_SDV import _compare_single_nba_game, plot_comparison
...
def main():
    raw = load_nba_data(seasons=[SEASON])
    slim = prepare_pbp(raw)
    game_results = extract_game_results(slim)
    elo_per_game, ratings, last_season = build_elo_ratings(game_results)
    feats = build_features(slim, league="NBA", elo_per_game=elo_per_game)

    results = train_and_evaluate(feats, league="NBA")
    ...
    for i, game_id in enumerate(sample_game_ids, start=1):
        ...
        try:
            comparison = _compare_single_nba_game(
                game_id=game_id,
                season=SEASON,
    ...
    per_game_df.to_csv("outputs/benchmark_per_game_summary.csv", index=False)
    print("\nSaved outputs/benchmark_per_game_summary.csv")
    ...
    plot_comparison(worst_comparison, worst_game_id, save_path="outputs/benchmark_worst_game.png")
